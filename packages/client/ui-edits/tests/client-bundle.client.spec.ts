 ...
 * not built (`pnpm --filter @deepseek-ai/dsh-client-ui-edits bundle`).
 */
...
const PLUGIN_ID = '@deepseek-ai/dsh-client-ui-edits'
...
function readBundle(): string | undefined {
  try {
    ...
    return readFileSync(resolve('packages/client/ui-edits/lib/client.js'), 'utf8')
  } catch {
    return undefined
  }
}
...
describe('tsdown client artifact', () => {
  ...
  it.skipIf(code === undefined)('registers the Edits tab into a real slot ring', async () => {
    ...
    // The 'edits' view entry is declared by the conversation slot package.
